...
df= pd.read_csv("/mount/src/fvue/app/datasets/Bank Account Fraud/bank_fraud_final.csv",nrows = 5000)
...
if st.button("Show Correleation matrix"):
    image = open('/mount/src/fvue/app/img/bank_fraud.png', 'rb').read()
    st.image(image, caption =  'Bank Fraud Correlation Matrix', use_column_width = True)
if st.button("Show Confusion Matrix"):
    image = open('/mount/src/fvue/app/img/bankruptcy_conf.png', 'rb').read()
...
if more:
    st.write("""
fraud_bool: This column indicates whether a transaction is fraudulent or not, typically represented as binary values (0 for non-fraudulent and 1 for fraudulent).

income: This column represents the income of the customer, usually in a monetary unit such as currency. Users should input numerical values representing the income.

name_email_similarity: This column measures the similarity between the customer's name and email address. Users should input numerical values representing the similarity.

prev_address_months_count: This column represents the number of months since the customer's previous address. Users should input numerical values representing the count.

current_address_months_count: This column represents the number of months since the customer's current address. Users should input numerical values representing the count.

customer_age: This column represents the age of the customer in years. Users should input numerical values representing the age.

days_since_request: This column represents the number of days since the request was made. Users should input numerical values representing the count of days.

intended_balcon_amount: This column represents the intended amount for a balcony (possibly in a real estate transaction). Users should input numerical values representing the amount.

zip_count_4w: This column represents the count of ZIP codes in the last 4 weeks. Users should input numerical values representing the count.

velocity_6h: This column represents the velocity of transactions in the last 6 hours. Users should input numerical values representing the velocity.
             
velocity_24h: This column represents the velocity of transactions in the last 24 hours. Users should input numerical values representing the velocity.

velocity_4w: This column represents the velocity of transactions in the last 4 weeks. Users should input numerical values representing the velocity.

bank_branch_count_8w: This column represents the count of bank branches visited in the last 8 weeks. Users should input numerical values representing the count.

date_of_birth_distinct_emails_4w: This column represents the count of distinct email addresses associated with the customer's date of birth in the last 4 weeks. Users should input numerical values representing the count.

credit_risk_score: This column represents the credit risk score of the customer. Users should input numerical values representing the score.

email_is_free: This column indicates whether the customer's email address is from a free email provider (e.g., Gmail, Yahoo). Users should input binary values (0 for not free, 1 for free).

phone_home_valid: This column indicates whether the customer's home phone number is valid. Users should input binary values (0 for invalid, 1 for valid).

phone_mobile_valid: This column indicates whether the customer's mobile phone number is valid. Users should input binary values (0 for invalid, 1 for valid).

bank_months_count: This column represents the count of months the customer has been with the bank. Users should input numerical values representing the count.

has_other_cards: This column indicates whether the customer has other cards. Users should input binary values (0 for no, 1 for yes).

proposed_credit_limit: This column represents the proposed credit limit for the customer. Users should input numerical values representing the limit.

foreign_request: This column indicates whether the request is from a foreign entity. Users should input binary values (0 for no, 1 for yes).

session_length_in_minutes: This column represents the length of the session in minutes. Users should input numerical values representing the length.

keep_alive_session: This column indicates whether the session is kept alive. Users should input binary values (0 for no, 1 for yes).

device_distinct_emails_8w: This column represents the count of distinct email addresses associated with the customer's device in the last 8 weeks. Users should input numerical values representing the count.

device_fraud_count: This column represents the count of fraud cases associated with the customer's device. Users should input numerical values representing the count.

month: This column represents the month of the transaction or request. Users should input numerical values representing the month (e.g., January as 1, February as 2, and so on).
