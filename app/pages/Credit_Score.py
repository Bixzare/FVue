...
if st.button("Show Correleation matrix"):
    image = open('/mount/src/fvue/app/img/credit_score_matrix.png', 'rb').read()
    st.image(image, caption =  'Income Correlation Matrix', use_column_width = True)
if st.button("Show Confusion Matrix"):
    image = open('/mount/src/fvue/app/img/credit_score_conf.png', 'rb').read()
...
if more:
    st.write("""
Month: Categorical value representing the month of the year.
Age: Numeric value representing the age of the customer.
Occupation: Categorical value representing the occupation of the customer.
Annual_Income: Numeric value representing the annual income of the customer.
Monthly_Inhand_Salary: Numeric value representing the monthly salary after deductions.
Num_Bank_Accounts: Numeric value representing the number of bank accounts the customer holds.
Num_Credit_Card: Numeric value representing the number of credit cards owned by the customer.
Interest_Rate: Numeric value representing the interest rate applicable to the customer's loans.
Num_of_Loan: Numeric value representing the number of loans the customer has.
Delay_from_due_date: Numeric value representing the delay from the due date for payments.
Num_of_Delayed_Payment: Numeric value representing the number of delayed payments made by the customer.
Changed_Credit_Limit: Numeric value representing the changes in the credit limit.
Num_Credit_Inquiries: Numeric value representing the number of credit inquiries made by the customer.
Credit_Mix: Categorical value representing the mix of credit products used by the customer.
Outstanding_Debt: Numeric value representing the outstanding debt amount.
Credit_Utilization_Ratio: Numeric value representing the credit utilization ratio of the customer.
Credit_History_Age: Numeric value representing the age of the credit history.
Payment_of_Min_Amount: Categorical value indicating whether the minimum payment amount is paid.
Total_EMI_per_month: Numeric value representing the total Equated Monthly Installment (EMI) paid by the customer.
Amount_invested_monthly: Numeric value representing the amount invested monthly by the customer.
Payment_Behaviour: Categorical value representing the payment behavior of the customer.
Monthly_Balance: Numeric value representing the monthly balance maintained by the customer.
Credit_Score: Numeric value representing the credit score of the customer.
Personal Loan: Binary value indicating whether the customer has a personal loan.
Credit-Builder Loan: Binary value indicating whether the customer has a credit-builder loan.
Debt Consolidation Loan: Binary value indicating whether the customer has a debt consolidation loan.
Mortgage Loan: Binary value indicating whether the customer has a mortgage loan.
Student Loan: Binary value indicating whether the customer has a student loan.
Payday Loan: Binary value indicating whether the customer has a payday loan.
Auto Loan: Binary value indicating whether the customer has an auto loan.
Home Equity Loan: Binary value indicating whether the customer has a home equity loan.""")
